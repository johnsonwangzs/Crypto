...
def crypt(plainTxt, key, cryptMode):  # 加解密
    global rk, rk1, X
    genKey(key)
    # 若为解密，密钥反序
...
if __name__ == '__main__':
    ...
    #     c = crypt(plainTxt, key, 0)
    # elif mode == 1:
    #     cipherTxt = eval(input("请输入128bit密文（16进制）："))
    #     m = crypt(cipherTxt, key, 1)
    '''
    加解密模式：cryptMode=0 -->加密  cryptMode=1 -->解密
    '''

    # 测试样例
    key = 0x0123456789ABCDEFFEDCBA9876543210
    plainTxt = 0x0123456789ABCDEFFEDCBA9876543210
    cipherTxt = 0x681EDF34D206965E86B3E94F536E4246
    cryptMode = 1
    graphviz = GraphvizOutput()
    graphviz.output_file = 'SM4.png'
    with PyCallGraph(output=graphviz):
        crypt(plainTxt, key, cryptMode)

    # 测试样例
    key = 0x0123456789ABCDEFFEDCBA9876543210
    plainTxt = 0x0123456789ABCDEF0808080808080808
    cryptMode = 0
    crypt(plainTxt, key, cryptMode)
