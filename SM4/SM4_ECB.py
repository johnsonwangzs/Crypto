...
if __name__ == '__main__':
    ...
    if mode == 0:
        ...
        for i in range(num + 1):
            cipherTxt.append(crypt(plainTxt[i], key, 0))
        for i in range(num + 1):
            for j in range(16):
        ...
        # 读入文件，取得密文
        fin = open("cipher-ecb", "rb")
        fout = open("plain-ecb_test", "wb")
        content = fin.read()
        cnt, num, tmp = 0, 0, 0
        cipherTxt = []
        for aByte in content:
        ...
        plainTxt = []
        for i in range(num):
            plainTxt.append(crypt(cipherTxt[i], key, 1))
        for i in range(num - 1):
            for j in range(16):
