...
def getBigPrime():
    ...
    while True:
        if Miller_Rabin.miller_rabin_api(rnd):
            return rnd
        rnd += 2
...
def RSA_encrypt(plaintext, pk, N):
    ...
    :return: 密文
    """
    ciphertext = fast_powMod.fast_pow_mod_montgomery(plaintext, pk, N)
    return ciphertext
...
def RSA_decrypt(ciphertext, sk, p, q):
    ...
    # 使用CRT加速解密过程
    m1 = fast_powMod.fast_pow_mod_montgomery(ciphertext, sk % (p - 1), p)
    m2 = fast_powMod.fast_pow_mod_montgomery(ciphertext, sk % (q - 1), q)
